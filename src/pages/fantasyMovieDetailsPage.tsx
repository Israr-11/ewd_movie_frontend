...
const FantasyMovieDetailsPage = () => {
  ...
  const { movies, isLoading, error, loadMovies, removeMovie, addCastToMovie } = useFantasyMovies();
  const [movie, setMovie] = useState<any>(null);
  
  // Dialog state for adding cast
  ...
  // Find the movie by ID once movies are loaded
  useEffect(() => {
  ...
  const handleAddCast = async () => {
    ...
    try {
      ...
      toast.error(`Failed to add cast member: ${err instanceof Error ? err.message : 'Unknown error'}`);
    } finally {
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
                  ...
                  <React.Fragment key={index}>
                    ...
                    <ListItem alignItems="flex-start">
                      ...
                      <ListItemText
                        ...
                        secondary={castMember.Description || 'No description provided'}
                      />
                    </ListItem>
                  </React.Fragment>
                ))}
      ...
      {/* Dialog for adding new cast member */}
