...
const HomePage: React.FC = () => {
  ...
  const changeFilterValues = (type: string, value: string) => {
    ...
    const filterIndex = filterValues.findIndex(
  ...
  const sortedMovies = [...filteredMovies].sort((a, b) => {
    const sortBy =
      filterValues.find((f) => f.name === "sortBy")?.value || "popularity.desc";
    ...
    <>
      ...
      {sortedMovies.length > moviesShown && (
        <Box sx={{ display: "flex", justifyContent: "center", my: 4 }}>
          <LoadMoreButton variant="contained" onClick={loadMoreMovies}>
            Load More Movies
          </LoadMoreButton>
        </Box>
      )}
